...
stra.srs.total2=function(Nh, mysample, stra.index, alpha)
{
  stra.num=length(Nh)
  Wh=Nh/sum(Nh)
  N = sum(Nh)
  ...
  total.est=sum(Wh*yh.est)*N
  total.var=sum(Wh^2*yh.var)*N^2
  total.sd =sqrt(total.var)

  ci.result=conf.interval(total.est, total.sd, alpha)
  total.left =ci.result$left
  total.right=ci.result$right

  total.result=matrix(c(total.est, total.var, total.sd, total.left, total.right), nrow=1)
  colnames(total.result)=c("total.est", "total.var", "total.sd", "total.left", "total.right")
  rownames(total.result)="Total"

  return(list(stra.result=as.data.frame(stra.result), total.result=as.data.frame(total.result)))
}